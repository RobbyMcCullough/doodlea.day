...
description: >
  Video-first BlockFrame structure remixed onto Doodle a Day. The unit is the frame (1080×1080).
  Atoms are thick ink borders, hard offset shadows, warm paper, Caveat Brush display, DM Sans body
  and chrome, square cards, label pills, tilted paper pieces, star bursts, stripe blocks, and dot grids.
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "DM Sans", cqw: 0.95, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "Caveat Brush", cqw: 1.15, weight: 400, upper: false, lineHeight: 1.2 }
  label:   { fontFamily: "DM Sans", px: 13, weight: 600, tracking: "0.08em", upper: true }
  counter: { fontFamily: "DM Sans", px: 14, weight: 700, tracking: "0.1em", upper: true }
  # — display ramp (Caveat Brush 800–900, uppercase, negative tracking) —
  heading-md:{ fontFamily: "Caveat Brush", cqw: 2.1, weight: 400, lineHeight: 1.1, tracking: "-0.01em", upper: false }
  quote-text:{ fontFamily: "Caveat Brush", cqw: 2.7, weight: 400, lineHeight: 1.15, tracking: "-0.02em", upper: false }
  stat-number:{ fontFamily: "Caveat Brush", cqw: 3.3, weight: 400, lineHeight: 1.0 }
  heading-lg:{ fontFamily: "Caveat Brush", cqw: 3.3, weight: 400, lineHeight: 1.0, tracking: "-0.02em", upper: false }
  close-title:{ fontFamily: "Caveat Brush", cqw: 4.2, weight: 400, lineHeight: 0.95, tracking: "-0.03em", upper: false }
  heading-xl:{ fontFamily: "Caveat Brush", cqw: 5.0, weight: 400, lineHeight: 0.95, tracking: "-0.03em", upper: false }
...
- **Fonts** — **Caveat Brush** is display only; **DM Sans** is body, caption, label, counter, and button chrome. Ignore any preset font name lingering in prose.
- **Weights** — Caveat Brush ships at 400; DM Sans ships at 400/600/700. Ignore the preset's heavier display-weight language below.
...
The voice is **Caveat Brush** at weight 800–900 in tight uppercase with negative tracking (display) +
...
- **Caveat Brush 800–900 uppercase** negative-tracked display; **Space Grotesk** label chrome.
...
- **Squint** — one Caveat Brush display moment dominates at 3–6× its neighbor; cards read as a system, not rivals.
...
Two ramps. The **reading/chrome ramp** (Caveat Brush body 0.95cqw weight 500, Space Grotesk labels in px)
carries copy + chrome; the **display ramp** (Caveat Brush `heading-md` 2.1cqw → `heading-xl` 5cqw, weight
...
- **Caveat Brush display is uppercase, weight 800–900, negative-tracked** (−0.01..−0.03em); body weight 500 sentence case; **Space Grotesk labels uppercase 0.08em**. No sentence-case display, no untracked display.
...
**Ground** `{colors.cream}` (or offwhite) + faint dot-grid. **Composes** label-pill, heading-xl, tilt-deco rect, star-burst, counter. **Focal** a 2–3 line Caveat Brush `heading-xl` uppercase, left, under a label-pill; tilted pastel rects + a star burst puncture the right. **Chrome** counter pill. **Accent** the decorations' pastels. **Silence** right third holds decorations. **Fixed** 4px/8px coupling, uppercase display, square. **Free** title, decoration placement/colors. **Density** comfortable.
...
**Ground** `{colors.pink}` (or offwhite). **Composes** label-pill, quote-text in a card-elevated, attribution. **Focal** an Caveat Brush `quote-text` (900 uppercase) inside a white bordered+shadowed frame. **Accent** the ground + one decoration. **Silence** moderate. **Fixed** uppercase quote, 4px/8px. **Free** quote, ground. **Density** comfortable.
...
- Set Caveat Brush display **uppercase, 800–900, negative-tracked**; open every region with a **label-pill**.
...
- No sentence-case Caveat Brush display; no untracked display; no label as plain text.
...
- **Squint** — one Caveat Brush display moment dominates; cards read as a system.
...
- **Type** — Caveat Brush uppercase 800–900 negative-tracked, fit-to-measure; Space Grotesk labels 0.08em; ≥1.4cqw floor.
...
- **Caveat Brush + Space Grotesk via Google Fonts.** CJK: Noto Sans SC 900 (sentence case — the uppercase signal drops); lean harder on borders/shadows/decoration to carry the brutalist identity.
